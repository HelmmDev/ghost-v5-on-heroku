...
/*Search Modify URL */
(function() {
    // Function to replace URLs in the search results
    function modifySearchResults(results) {
        return results.map(result => {
            if (result.url) {
                result.url = result.url.replace("http://localhost:2368", "https://www.helmm.com/lifeathelmm");
            }
            return result;
        });
    }

    // Attach the search functionality after the search box is loaded
    document.addEventListener('click', function(event) {
        var searchIcon = event.target.closest('.search-icon');
        if (searchIcon) {
            // Check if the search input exists after the search box is loaded
            setTimeout(function() {
                var searchInput = document.querySelector('input[placeholder="Search posts, tags and authors"]');
                if (searchInput) {
                    searchInput.addEventListener('input', function(event) {
                        // Assuming the search function from search-index.js is invoked here
                        var searchResults = searchIndex.search(event.target.value); // This line assumes that `searchIndex.search()` is accessible

                        // Modify URLs in search results
                        searchResults.posts = modifySearchResults(searchResults.posts);
                        searchResults.authors = modifySearchResults(searchResults.authors);
                        searchResults.tags = modifySearchResults(searchResults.tags);

                        // Assuming there's a function in search-index.js that displays these results
                        displaySearchResults(searchResults); // Make sure this matches your actual implementation
                    });
                } else {
